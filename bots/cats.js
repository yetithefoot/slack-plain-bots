...
var execute = function(req, res, next) {
	...
	var catTexts = catPhrases[(req.params.locale || "en")];
	...
	var text = "Я тут что-то разчехлиться не могу! Позовите @yetithefoot! Хозяиииин? :scream_cat:";
	switch (mode) {
		case "text":
			var randomindex = Math.floor(Math.random() * catTexts.length);
			text = catTexts[randomindex];
			break;
		case "answer": // just add username to text
			var randomindex = Math.floor(Math.random() * catTexts.length);
			text = "@" + (req.param("user_name") || "everyone") + " " + catTexts[randomindex];
			break;
		case "image":
			// thecat api images
...
module.exports = exports = function(router) {
	router.post("/cats/:locale?", execute);
};